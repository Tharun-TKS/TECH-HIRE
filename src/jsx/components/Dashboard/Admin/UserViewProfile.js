...
import { getSeekerById } from '../../../../services/Seekerservice';
        ...
        const fetchData = async () => {
            try {
                ...
                if (userResponse.role === 'Seeker') {
                    const seekerResponse = await getSeekerById(id);
                    setRoleData(seekerResponse.data || []);
                } else if (userResponse.role === 'Employer') {
                    const employerResponse = await getEmployerById(id);
                    setRoleData(employerResponse.data || []);
                }
                setLoading(false);
            } catch (error) {
					...
					<div className="card">
						<div className="card-header border-0 flex-wrap align-items-start">
							<div className="col-md-8">
								<div className="user d-sm-flex d-block pe-md-5 pe-0">
									...
									<div className="ms-sm-3 ms-0 me-md-5 md-0">
										<h5 className="mb-1 font-w600"><Link to={"#"}>{userData.username}</Link></h5>
										<div className="listline-wrapper mb-2">
											<span className="item"><i className="text-primary far fa-envelope"></i>{userData.email}</span>
											<span className="item"><i className="text-primary far fa-id-badge"></i>Manager</span>
											<span className="item"><i className="text-primary fas fa-map-marker-alt"></i>{roleData}</span>
										</div>
							...
							<div className="col-md-4 col-12 text-end">	
								<Link to={"#"} className="btn btn-sm btn-primary me-2">Hire</Link>
								<div className="mt-3">
