...
const SeekerJobList = () => {
  ...
  useEffect(() => {
    const loadJobsAndOrganizations = async () => {
      try {
        ...
        // Map jobs to include organization details
        const jobsWithOrganizations = jobsData.map((job) => {
    ...
    loadJobsAndOrganizations();
  }, []);

  const navigate = useNavigate();

  const handleJobClick = (id) => {
    navigate(`/seeker-jobview/${id}`);
  };
  if (loading) {
    return <div>Loading...</div>; // Simple loading state, can be replaced with a spinner or loader component
  }
...
//                 <h5 className="card-title">No jobs currently available.</h5>
            ...
            <div className="row">
              {jobs.map((job) => (
                ...
                >
                  <div className="card">
                    <div
                      className="jobs2 card-body text-center"
                      onClick={() => handleJobClick(job.id)}
                    >
                      <div className="text-center">
                        <img src={pic1} alt="" className="mb-2" />
                        <h4 className="mb-0">
                          <Link to={"#"}>{job.organizationName}</Link>
                        </h4>
                        <span className="text-primary mb-3 d-block">
                          {job.jobpostingsubject}
                        </span>
                      </div>
                      <div>
                        <span className="d-block mb-1">
                          <i className="fas fa-map-marker-alt me-2"></i>
                          {job.organizationLocation}
                        </span>
                        <span>
                          <i className="fas fa-comments-dollar me-2"></i>
                          {job.jobsalaryoffered}
                        </span>
                      </div>
                    </div>
                  </div>
