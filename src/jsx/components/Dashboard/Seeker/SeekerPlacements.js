...
import { fetchPlacementsBySeekerId } from '../../../../services/PlacementService';
import { useSelector } from 'react-redux';




const SeekerPlacements = () => {
 ...
 useEffect(() => {
      const fetchJobDetails = async () => {
        try {
          const jobDetails = await fetchPlacementsBySeekerId(id);
          setContacts(jobDetails.data);
        } catch (err) {
          setError('Error fetching job details.');
          swal({
            icon: 'error',
            title: 'Oops...',
            text: 'Error fetching job details!',
          });
        } finally {
          setLoading(false);
        }
						...
						<table className="table display mb-4 dataTablesCard job-table table-responsive-xl card-table dataTable no-footer" >
							<thead>
								<tr>
									...
									<th>Placed College</th>
                                    <th>Placed SeekerName</th>
									<th>Placed Salary</th>
									<th>Placed Charges</th>
                                    <th>Placed Status</th>
									<th>Actions</th>
								</tr>
							</thead>
							<tbody>
								{contacts.map((contact,index)=>(
									<tr key={index}>
										<td>{contact.id}</td>
										<td>{contact.seekerid}</td>
										<td>{contact.organizationid}</td>
										<td>{contact.jobid}</td>
										<td>{contact.placementdate}</td>
										<td>{contact.placementlocation}</td>
                                        <td>{contact.placedcollege}</td>
										<td>{contact.placedseekername}</td>
										<td>{contact.placedsalary}</td>
                                        <td>{contact.placedcharges}</td>
										<td><span className={`badge badge-lg light badge-${contact.placementstatus=== "true" ? 'success' : 'danger'}`}>{contact.placementstatus}</span></td>
										<td>
...
export default SeekerPlacements;