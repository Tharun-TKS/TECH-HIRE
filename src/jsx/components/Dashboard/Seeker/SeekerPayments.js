...
import { fetchPaymentBySeekerId } from "../../../../services/PaymentService";
...
const SeekerPayments = () => {
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        const jobDetails = await fetchPaymentBySeekerId(id);
            ...
            <table className="table display mb-4 dataTablesCard job-table table-responsive-xl card-table dataTable no-footer">
              ...
              <tbody>
                {contacts.map((contact, index) => (
                  <tr key={index}>
                    <td>{contact.id}</td>
                    <td>{contact.placementid}</td>
                    <td>{contact.seekerid}</td>
                    <td>{contact.referenceid}</td>
                    <td>{contact.paymentemailid}</td>
                    <td>{contact.paymentamount}</td>
                    <td>{contact.paymentdate}</td>
                    <td>
                      <span
                        className={`badge badge-lg light badge-${
                          contact.paymentstatus === "Pending" ? "success" : "danger"
                        }`}
                      >
